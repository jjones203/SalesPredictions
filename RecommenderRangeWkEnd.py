...
# stat_nbr, (year, month, day, week, weekend, avg temp)
weather_data = weather_data.map(lambda l: (int(l[0]), (int(l[1]), int(l[2]), int(l[3]), int(l[4]), int(l[5]), int(l[6]))))
...
# ^ now (store, (YY, MM, DD, week, weekend, avgTemp))
...
#[(store #, year, month, day), (week, weekend, temp)]
store_date_temp = store_date_temp.map(lambda l: ((l[0], l[1][0], l[1][1], l[1][2]), (l[1][3], l[1][4], l[1][5])))
sales_temp_data = sales_data.join(store_date_temp)
# ((store, year, month, date), ((item, sales), (week, weekend, temp))
ratings_RDD = sales_temp_data.map(lambda l: Rating(l[0][0]*1000+l[1][0][0], (l[1][1][2]//4)+1000*l[1][1][0]+100*l[1][1][1],\
                                    l[1][0][1]))
# ((store*1000+item, temp//15+1000*week+100*weekend, sales)
